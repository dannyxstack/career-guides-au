"""为加拿大(StatCan NOC 2021)职业批量生成核心数据并入库、出中英 markdown。

区别于 gen_ca_occupations.py（镜像 AU、由 LLM 反推 NOC 码，受碰撞限制只得 ~262 个）：
本脚本以 **官方 NOC 2021 五位 unit group 清单**（.codex_tmp/noc_target.json）直接驱动，
对尚未入库的每个 NOC 职业用 DeepSeek 生成：中文名、中英简介、教育路径、资质/执照、薪资(CAD)、
加拿大移民路径(Express Entry: FSW/CEC / 省提名 PNP / LMIA 工签 / Atlantic)、11 维评分(10 分制)、
适合人群、FAQ(中英)、增长热词，并从【现有 AU/US/UK/NZ/CA 候选池】中选最接近母体供 copy_ai_blocks。
只生成 zh-CN + en 两种语言；其余语言走后续统一翻译。

- 分类：LLM 从站点现有 11 个分类中择一。
- 移民语义(is_migration)：0=几乎无技术移民路径；1=可走 Express Entry(FSW/CEC) 或 PNP；2=受限/仅 LMIA 担保或特定省。
- ai-block 母体匹配写入 .codex_tmp/ca_ai_match.json（与 copy_ai_blocks --country CA 对接）。
- 幂等：处理过的 NOC 记到 .codex_tmp/ca2_done.json；已存在于 CA 则跳过。

运行：python -m scripts.gen_ca2_occupations [--batch-size 50] [--rest 0] [--limit N] [--redo]
"""
...
NOC_LIST = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "noc_target.json")
STATE = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "ca2_done.json")
MATCH = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "ca_ai_match.json")
...
def build_system(pool):
    ...
    listing = " | ".join(names)
    return (
        "你是加拿大劳动力市场与移民分析师，熟悉 StatCan NOC 2021 职业分类(TEER)、Job Bank 薪资与需求、"
        "加拿大技术移民(快速通道 Express Entry: FSW/CEC / 省提名 PNP / 大西洋移民 AIP / LMIA 雇主担保工签)。"
        "针对给定的加拿大 NOC 职业，输出务实、具体的加拿大数据。薪资用加元(CAD)整数年薪。"
        "中文面向国际读者，不要提『华人/中文社区』。\n"
        f"职业分类只能从以下 11 类中选一个（原文照填）：{cats}\n"
        "另需从下列【现有职业候选池】中选出与该 NOC 职业最接近的同一职业（用于复用分析），"
...
def build_prompt(code, title, definition):
    dims = "、".join(DIMS)
    return f"""加拿大 NOC 2021 职业：
- NOC 码：{code}
- 英文名：{title}
- 官方定义：{(definition or '')[:400]}

请输出该加拿大职业的 JSON，字段如下（全部必填）：
- name_zh：中文职业名
...
- is_migration：0/1/2（0=几乎无技术移民路径；1=可走 Express Entry(FSW/CEC) 或 PNP；2=受限/仅 LMIA 担保或特定省）
- shortage：0 或 1（是否加拿大紧缺/高需求职业，参考 Job Bank Outlook）
...
- qualifications：数组，每项{{qual_name:中文, issuer:机构, note:中文, is_mandatory:0或1}}（2-4 项，含执照/认证/学位/省级注册）
- salaries：数组，每项{{experience:中文如"初级（0-3年）", salary_min:整数CAD, salary_max:整数CAD, salary_note:中文}}（3 档：初/中/高）
- visa：数组，每项{{visa_subclass:如"Express Entry"/"FSW"/"CEC"/"PNP"/"LMIA", visa_name:英文, description:中文}}（2-4 项加拿大移民/工签路径）
...
- match_country：该匹配职业所属国家代码 "AU"/"US"/"UK"/"NZ"/"CA"；match_name 为空则填 ""
...
def save(code, title, v):
    OCC = {"country_code": "CA", "occ_code": code, "anzsco_code": code,
           "occ_code_type": "NOC", "anzsco_title": title, "category": v["category"],
    ...
    generate_md(code, locale="zh-CN", country="CA")
    generate_md(code, locale="en", country="CA")
    return occ_id
...
def run(batch_size, rest, limit, redo):
    noc_list = load_json(NOC_LIST, [])
    noc_list = [s for s in noc_list if "n.e.c" not in (s.get("title") or "").lower()]
    with get_cursor() as cur:
        cur.execute("SELECT o.id,o.country_code,o.occ_code,o.anzsco_title AS name_en "
                    "FROM occupations o WHERE o.country_code IN ('AU','CA','US','UK','NZ')")
        pool_rows = cur.fetchall()
        cur.execute("SELECT occ_code FROM occupations WHERE country_code='CA'")
        ca_codes = {r["occ_code"] for r in cur.fetchall()}
    ...
    targets = [s for s in noc_list if s["soc"] not in done and s["soc"] not in ca_codes]
    if limit:
        targets = targets[:limit]
    n = len(targets)
    nb = (n + batch_size - 1) // batch_size if batch_size else 1
    print(f"[ca2] 待生成 {n} 个(分 {nb} 批，每批 {batch_size}，批间休息 {rest}s) model={config.DEEPSEEK_MODEL}", flush=True)
    okc = fail = matched = 0
    for bi in range(nb):
        ...
        for idx, s in enumerate(chunk, 1):
            ...
            try:
                ...
                print(f"  [{tag}] → CA id={occ_id} {v['name_zh']} [{v['category']}] mig={v['is_migration']} {mtxt}", flush=True)
