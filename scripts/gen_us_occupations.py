"""为美国(SOC 2018)职业批量生成核心数据并入库、出中英 markdown。

对 SOC 2018 detailed 清单（.codex_tmp/soc_2018.json，已剔除 "All Other" 兜底类）中的每个职业，
用 DeepSeek 生成：中文名、中英简介、教育路径、资质/执照、薪资(USD)、工作签证/绿卡路径
(H-1B/EB-2/EB-3/O-1/TN/Green Card)、11 维评分(10 分制)、适合人群、FAQ(中英)、增长热词，
并从【现有 AU/CA 职业候选池】中选出最接近的母体（match_name/match_country，方式 a），
供后续复制 ai-block。经 seed_occupation_v2 入库，再 generate_md 出 zh-CN + en 两份 markdown。

注意：SOC 码取自官方清单；薪资/签证资格/评分均为 LLM 最佳估计，上线前需二次核对。
- 分类：LLM 从站点现有 11 个分类中择一。
- 移民语义(is_migration)：0=雇主担保困难/几乎无技术移民路径；1=常见 H-1B/EB-2/EB-3 绿卡路径；
  2=受限/配额紧张（如 H-1B cap 抽签）或仅特定路径。
- ai-block 母体匹配写入 .codex_tmp/us_ai_match.json：{us_occ_id: {country, occ_code}}（无匹配则不写）。
- 幂等：处理过的 SOC 记到 .codex_tmp/us_done.json，重跑跳过；SOC 已存在于 US 则跳过。
- 批次：--batch-size 每批(默认 50)，--rest 批间休息秒数(默认 1200=20 分钟)。

运行：python -m scripts.gen_us_occupations [--batch-size 50] [--rest 1200] [--limit N] [--redo]
"""
...
SOC_LIST = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "soc_2018.json")
STATE = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "us_done.json")
...
def build_system(pool):
    ...
    listing = " | ".join(names)
    return (
        "你是美国劳动力市场与移民分析师，熟悉 SOC 2018 职业分类、BLS OEWS 薪资、"
        "美国工作签证(H-1B/L-1/O-1/TN)与职业移民绿卡(EB-2/EB-3/PERM 劳工证)。"
        "针对给定的美国 SOC 职业，输出务实、具体的美国数据。薪资用美元(USD)整数年薪。"
...
请输出该美国职业的 JSON，字段如下（全部必填）：
- name_zh：中文职业名
- category：从给定 11 类中选一个（原文照填）
- summary_zh / summary_en：一句话简介（中/英，各 60-140 字/词）
- forecast_note_zh：美国就业前景一段（60-120 字）
- trend_summary_zh：职业发展/晋升路径一段（60-120 字）
- is_migration：0/1/2（0=雇主担保困难/几乎无技术移民路径；1=常见 H-1B/EB-2/EB-3 绿卡路径；2=受限/配额紧张或仅特定路径）
- shortage：0 或 1（是否美国紧缺/高需求职业）
- workforce_size：美国从业人数估计（整数）
- growth：增长/热点关键词英文数组（4 个）
- education：数组，每项{{stage:中文阶段, duration:如"4年", cost_min:整数USD, cost_max:整数USD, cost_note:中文}}（2-3 项）
- qualifications：数组，每项{{qual_name:中文, issuer:机构, note:中文, is_mandatory:0或1}}（2-4 项，含执照/认证/学位）
- salaries：数组，每项{{experience:中文如"初级（0-3年）", salary_min:整数USD, salary_max:整数USD, salary_note:中文}}（3 档：初/中/高）
- visa：数组，每项{{visa_subclass:如"H-1B"/"EB-2"/"EB-3"/"O-1"/"TN"/"Green Card (PERM)", visa_name:英文, description:中文}}（2-4 项美国工作签证/绿卡路径）
...
- match_country：该匹配职业所属国家代码 "AU" 或 "CA"；match_name 为空则填 ""
只输出真实合理的美国数据；不确定的薪资给保守区间。"""
...
def save(soc, title, v):
    ...
    OCC = {"country_code": "US", "occ_code": soc, "anzsco_code": soc,
           "occ_code_type": "SOC", "anzsco_title": title, "category": v["category"],
           "currency": "USD", "workforce_size": v["workforce_size"], "shortage_listed": v["shortage"],
    ...
    src = [{"source_name": "BLS OEWS", "content": "SOC 薪资与需求", "url": "https://www.bls.gov/oes/"}]
    ...
    generate_md(soc, locale="zh-CN", country="US")
    generate_md(soc, locale="en", country="US")
    return occ_id
...
def run(batch_size, rest, limit, redo):
    ...
    soc_list = [s for s in soc_list if not s["title"].lower().endswith("all other")]
    ...
    with get_cursor() as cur:
        cur.execute("SELECT o.id,o.country_code,o.occ_code,o.anzsco_title AS name_en "
                    "FROM occupations o WHERE o.country_code IN ('AU','CA')")
        pool_rows = cur.fetchall()
        cur.execute("SELECT occ_code FROM occupations WHERE country_code='US'")
        us_codes = {r["occ_code"] for r in cur.fetchall()}
    ...
    targets = [s for s in soc_list if s["soc"] not in done and s["soc"] not in us_codes]
    if limit:
        targets = targets[:limit]
    n = len(targets)
    nb = (n + batch_size - 1) // batch_size if batch_size else 1
    print(f"[us] 待生成 {n} 个(分 {nb} 批，每批 {batch_size}，批间休息 {rest}s) model={config.DEEPSEEK_MODEL}", flush=True)
    okc = fail = matched = 0
    for bi in range(nb):
        ...
        for idx, s in enumerate(chunk, 1):
            ...
            try:
                ...
                print(f"  [{tag}] → US id={occ_id} {v['name_zh']} [{v['category']}] mig={v['is_migration']} {mtxt}", flush=True)
