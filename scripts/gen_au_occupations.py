"""为澳洲(ABS ANZSCO)职业批量生成核心数据并入库、出中英 markdown。

对 ANZSCO unit group 清单（.codex_tmp/anzsco_target.json，已含现有 AU 码以便跳过）中
尚未入库的每个职业，用 DeepSeek 生成：中文名、中英简介、教育路径、资质/执照、薪资(AUD)、
澳洲签证/移民路径(Skills in Demand 482 / ENS 186 / 技术独立 189 / 州担保 190 / 偏远 491 / DAMA)、
11 维评分(10 分制)、适合人群、FAQ(中英)、增长热词，并从【现有 CA/US/UK/AU 职业候选池】中选出
最接近的母体（match_name/match_country，方式 a），供后续 copy_ai_blocks 复制 ai-block。
经 seed_occupation_v2 入库，再 generate_md 出 zh-CN + en 两份 markdown。

注意：ANZSCO 码取自 ABS 官方清单；薪资/签证资格/评分均为 LLM 最佳估计，上线前需二次核对。
- 分类：LLM 从站点现有 11 个分类中择一。
- 移民语义(is_migration)：0=几乎无技术移民路径；1=可走技术移民(在 CSOL/MLTSSL 上，189/190/482→186)；
  2=受限（仅特定州担保/偏远/DAMA 或高门槛）。
- ai-block 母体匹配写入 .codex_tmp/au_ai_match.json：{au_occ_id: {country, occ_code, src_occ_id}}（无匹配则不写）。
- 幂等：处理过的 ANZSCO 记到 .codex_tmp/au_done.json，重跑跳过；已存在于 AU 则跳过。
- 批次：--batch-size 每批(默认 50)，--rest 批间休息秒数(默认 0)。

运行：python -m scripts.gen_au_occupations [--batch-size 50] [--rest 0] [--limit N] [--redo]
"""
...
ANZSCO_LIST = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "anzsco_target.json")
STATE = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "au_done.json")
MATCH = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "au_ai_match.json")
...
def build_system(pool):
    ...
    listing = " | ".join(names)
    return (
        "你是澳洲劳动力市场与移民分析师，熟悉 ABS ANZSCO 职业分类、Jobs and Skills Australia 需求数据、"
        "澳洲技术与雇主担保签证(Skills in Demand 482 / 雇主提名 ENS 186 / 技术独立 189 / 州担保 190 / "
        "偏远地区 491·494 / DAMA)及技术职业清单(Core Skills Occupation List CSOL / MLTSSL)。"
        "针对给定的澳洲 ANZSCO 职业，输出务实、具体的澳洲数据。薪资用澳元(AUD)整数年薪。"
        "中文面向国际读者，不要提『华人/中文社区』。\n"
        f"职业分类只能从以下 11 类中选一个（原文照填）：{cats}\n"
        "另需从下列【现有职业候选池】中选出与该 ANZSCO 职业最接近的同一职业（用于复用分析），"
...
def build_prompt(code, title, definition):
    dims = "、".join(DIMS)
    return f"""澳洲 ANZSCO 职业：
- ANZSCO 码：{code}
- 英文名：{title}
- 官方定义：{(definition or '')[:400]}

请输出该澳洲职业的 JSON，字段如下（全部必填）：
- name_zh：中文职业名
- category：从给定 11 类中选一个（原文照填）
- summary_zh / summary_en：一句话简介（中/英，各 60-140 字/词）
- forecast_note_zh：澳洲就业前景一段（60-120 字）
- trend_summary_zh：职业发展/晋升路径一段（60-120 字）
- is_migration：0/1/2（0=几乎无技术移民路径；1=可走技术移民(在 CSOL/MLTSSL 上，189/190/482→186)；2=受限/仅特定州担保或偏远/DAMA/高门槛）
- shortage：0 或 1（是否澳洲紧缺/高需求职业，参考 JSA Skills Priority List）
- workforce_size：澳洲从业人数估计（整数）
- growth：增长/热点关键词英文数组（4 个）
- education：数组，每项{{stage:中文阶段, duration:如"4年", cost_min:整数AUD, cost_max:整数AUD, cost_note:中文}}（2-3 项）
- qualifications：数组，每项{{qual_name:中文, issuer:机构, note:中文, is_mandatory:0或1}}（2-4 项，含执照/注册/学位/技能评估）
- salaries：数组，每项{{experience:中文如"初级（0-3年）", salary_min:整数AUD, salary_max:整数AUD, salary_note:中文}}（3 档：初/中/高）
- visa：数组，每项{{visa_subclass:如"482"/"186"/"189"/"190"/"491", visa_name:英文, description:中文}}（2-4 项澳洲签证/移民路径）
...
- match_country：该匹配职业所属国家代码 "CA"/"US"/"UK"/"AU"；match_name 为空则填 ""
只输出真实合理的澳洲数据；不确定的薪资给保守区间。"""
...
def validate(raw):
    ...
    if cat not in CATEGORIES:
        cat = "Business, Finance & Legal"  # 兜底，避免分类越界
    out = {
        "name_zh": g("name_zh"), "category": cat,
...
def gen(system, code, title, definition):
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": system},
                  {"role": "user", "content": build_prompt(code, title, definition)}],
...
def save(code, title, v):
    """入库 + 英文 FAQ + 中英 md。返回 occ_id。"""
    OCC = {"country_code": "AU", "occ_code": code, "anzsco_code": code,
           "occ_code_type": "ANZSCO", "anzsco_title": title, "category": v["category"],
           "currency": "AUD", "workforce_size": v["workforce_size"], "shortage_listed": v["shortage"],
    ...
    EN = {"locale": "en", "name": title, "summary": v["summary_en"],
          "forecast_note": v["forecast_note_zh"], "trend_summary": v["trend_summary_zh"]}
    faqs_zh = [{"faq_type": f["faq_type"], "sort_order": f["sort_order"],
                "question": f["question_zh"], "answer": f["answer_zh"]} for f in v["faqs"]]
    src = [{"source_name": "Jobs and Skills Australia", "content": "ANZSCO 薪资与需求", "url": "https://www.jobsandskills.gov.au/"}]
    ...
    generate_md(code, locale="zh-CN", country="AU")
    generate_md(code, locale="en", country="AU")
    return occ_id
...
def run(batch_size, rest, limit, redo):
    anzsco_list = load_json(ANZSCO_LIST, [])
    anzsco_list = [s for s in anzsco_list if "n.e.c" not in (s.get("title") or "").lower()]  # 剔除兜底类
    # ai-block 母体候选池（已有 ai-block 的他国/现有 AU 职业），用于匹配复用
    with get_cursor() as cur:
        ...
        cur.execute("SELECT occ_code FROM occupations WHERE country_code='AU'")
        au_codes = {r["occ_code"] for r in cur.fetchall()}
    pool = [(r["country_code"], r["occ_code"], r["name_en"], r["id"]) for r in pool_rows]
    resolver = {}       # norm_name -> (country, occ_code, occ_id)
    resolver_cc = {}    # (country, norm_name) -> (country, occ_code, occ_id)
    ...
    targets = [s for s in anzsco_list if s["soc"] not in done and s["soc"] not in au_codes]
    if limit:
        targets = targets[:limit]
    n = len(targets)
    nb = (n + batch_size - 1) // batch_size if batch_size else 1
    print(f"[au] 待生成 {n} 个(分 {nb} 批，每批 {batch_size}，批间休息 {rest}s) model={config.DEEPSEEK_MODEL}", flush=True)
    okc = fail = matched = 0
    for bi in range(nb):
        ...
        for idx, s in enumerate(chunk, 1):
            ...
            try:
                ...
                print(f"  [{tag}] → AU id={occ_id} {v['name_zh']} [{v['category']}] mig={v['is_migration']} {mtxt}", flush=True)
