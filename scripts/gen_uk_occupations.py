"""为英国(ONS SOC 2020)职业批量生成核心数据并入库、出中英 markdown。

对 SOC 2020 unit group 清单（.codex_tmp/soc_2020.json，运行时剔除 "n.e.c." 兜底类）中的每个职业，
用 DeepSeek 生成：中文名、中英简介、教育路径、资质/执照、薪资(GBP)、英国工作签证/居留路径
(Skilled Worker / Health and Care Worker / Global Talent / Scale-up / 紧缺职业清单)、
11 维评分(10 分制)、适合人群、FAQ(中英)、增长热词，并从【现有 AU/CA/US 职业候选池】中选出
最接近的母体（match_name/match_country，方式 a），供后续复制 ai-block。
经 seed_occupation_v2 入库，再 generate_md 出 zh-CN + en 两份 markdown。

注意：SOC 码取自 ONS 官方清单；薪资/签证资格/评分均为 LLM 最佳估计，上线前需二次核对。
- 分类：LLM 从站点现有 11 个分类中择一。
- 移民语义(is_migration)：0=雇主难担保/几乎无技术移民路径；1=可走 Skilled Worker 担保（在可担保职业清单上）；
  2=受限（薪资门槛高/仅特定路径/紧缺豁免）。
- ai-block 母体匹配写入 .codex_tmp/uk_ai_match.json：{uk_occ_id: {country, occ_code, src_occ_id}}（无匹配则不写）。
- 幂等：处理过的 SOC 记到 .codex_tmp/uk_done.json，重跑跳过；SOC 已存在于 UK 则跳过。
- 批次：--batch-size 每批(默认 50)，--rest 批间休息秒数(默认 0)。

运行：python -m scripts.gen_uk_occupations [--batch-size 50] [--rest 0] [--limit N] [--redo]
"""
...
SOC_LIST = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "soc_2020.json")
STATE = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "uk_done.json")
MATCH = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "uk_ai_match.json")
...
def build_system(pool):
    ...
    listing = " | ".join(names)
    return (
        "你是英国劳动力市场与移民分析师，熟悉 ONS SOC 2020 职业分类、ASHE 薪资数据、"
        "英国工作签证与居留路径(Skilled Worker / Health and Care Worker / Global Talent / "
        "Scale-up / Graduate)及紧缺职业清单(Immigration Salary List)。"
        "针对给定的英国 SOC 职业，输出务实、具体的英国数据。薪资用英镑(GBP)整数年薪。"
        "中文面向国际读者，不要提『华人/中文社区』。\n"
        f"职业分类只能从以下 11 类中选一个（原文照填）：{cats}\n"
        "另需从下列【现有职业候选池】中选出与该 SOC 职业最接近的同一职业（用于复用分析），"
...
def build_prompt(soc, title, definition):
    dims = "、".join(DIMS)
    return f"""英国 SOC 2020 职业：
- SOC 码：{soc}
- 英文名：{title}
- 官方定义：{(definition or '')[:400]}

请输出该英国职业的 JSON，字段如下（全部必填）：
- name_zh：中文职业名
- category：从给定 11 类中选一个（原文照填）
- summary_zh / summary_en：一句话简介（中/英，各 60-140 字/词）
- forecast_note_zh：英国就业前景一段（60-120 字）
- trend_summary_zh：职业发展/晋升路径一段（60-120 字）
- is_migration：0/1/2（0=雇主难担保/几乎无技术移民路径；1=可走 Skilled Worker 担保（在可担保职业清单上）；2=受限/薪资门槛高/仅特定路径）
- shortage：0 或 1（是否英国紧缺/高需求职业，参考 Immigration Salary List）
- workforce_size：英国从业人数估计（整数）
- growth：增长/热点关键词英文数组（4 个）
- education：数组，每项{{stage:中文阶段, duration:如"4年", cost_min:整数GBP, cost_max:整数GBP, cost_note:中文}}（2-3 项）
- qualifications：数组，每项{{qual_name:中文, issuer:机构, note:中文, is_mandatory:0或1}}（2-4 项，含执照/认证/学位）
- salaries：数组，每项{{experience:中文如"初级（0-3年）", salary_min:整数GBP, salary_max:整数GBP, salary_note:中文}}（3 档：初/中/高）
- visa：数组，每项{{visa_subclass:如"Skilled Worker"/"Health & Care"/"Global Talent"/"Scale-up"/"Graduate", visa_name:英文, description:中文}}（2-4 项英国工作签证/居留路径）
...
- match_country：该匹配职业所属国家代码 "AU"/"CA"/"US"；match_name 为空则填 ""
只输出真实合理的英国数据；不确定的薪资给保守区间。"""
...
def gen(system, soc, title, definition):
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": system},
                  {"role": "user", "content": build_prompt(soc, title, definition)}],
...
def save(soc, title, v):
    """入库 + 英文 FAQ + 中英 md。返回 occ_id。"""
    OCC = {"country_code": "UK", "occ_code": soc, "anzsco_code": soc,
           "occ_code_type": "SOC", "anzsco_title": title, "category": v["category"],
           "currency": "GBP", "workforce_size": v["workforce_size"], "shortage_listed": v["shortage"],
    ...
    src = [{"source_name": "ONS ASHE", "content": "SOC 薪资与需求", "url": "https://www.ons.gov.uk/employmentandlabourmarket"}]
    ...
    generate_md(soc, locale="zh-CN", country="UK")
    generate_md(soc, locale="en", country="UK")
    return occ_id
...
def run(batch_size, rest, limit, redo):
    ...
    soc_list = [s for s in soc_list if "n.e.c" not in s["title"].lower()]  # 剔除兜底类
    # ai-block 母体候选池 + 解析索引（normalized name_en -> (country, occ_code, occ_id)）
    with get_cursor() as cur:
        cur.execute("SELECT o.id,o.country_code,o.occ_code,o.anzsco_title AS name_en "
                    "FROM occupations o WHERE o.country_code IN ('AU','CA','US')")
        pool_rows = cur.fetchall()
        cur.execute("SELECT occ_code FROM occupations WHERE country_code='UK'")
        uk_codes = {r["occ_code"] for r in cur.fetchall()}
    ...
    targets = [s for s in soc_list if s["soc"] not in done and s["soc"] not in uk_codes]
    if limit:
        targets = targets[:limit]
    n = len(targets)
    nb = (n + batch_size - 1) // batch_size if batch_size else 1
    print(f"[uk] 待生成 {n} 个(分 {nb} 批，每批 {batch_size}，批间休息 {rest}s) model={config.DEEPSEEK_MODEL}", flush=True)
    okc = fail = matched = 0
    for bi in range(nb):
        ...
        for idx, s in enumerate(chunk, 1):
            ...
            try:
                ...
                print(f"  [{tag}] → UK id={occ_id} {v['name_zh']} [{v['category']}] mig={v['is_migration']} {mtxt}", flush=True)
