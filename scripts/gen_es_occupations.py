"""为西班牙(CNO-2011)职业批量生成核心数据并入库、出中英 markdown。

对 CNO-2011 4 位 grupos primarios 清单（.codex_tmp/soc_es.json，código + 西语名）中的每个职业，
用 DeepSeek 生成：英文职业名、中文名、中英简介、教育路径、资质/认证、薪资(EUR)、西班牙工作/居留路径
(Tarjeta azul UE / Profesional altamente cualificado Ley 14/2013 / Cuenta ajena / 学历认证 homologación)、
11 维评分(10 分制)、适合人群、FAQ(中英)、增长热词，并从【现有 AU/CA/US/UK 职业候选池】中选出
最接近的母体（match_name/match_country），供后续复制 ai-block。
经 seed_occupation_v2 入库，再 generate_md 出 zh-CN + en 两份 markdown。

注意：CNO 标题为西文，英文名(name_en)由 LLM 产出；薪资/签证资格/评分均为 LLM 最佳估计，上线前需二次核对。
- 分类：LLM 从站点现有 11 个分类中择一。
- 移民语义(is_migration)：考虑欧盟自由流动——0=第三国难担保/几乎无技术移民路径；
  1=可走 Tarjeta azul UE / 高技能专业人才(Ley 14/2013) / 紧缺职业担保；2=受限（需学历认证 homologación/受规管职业/门槛高）。
- ai-block 母体匹配写入 .codex_tmp/es_ai_match.json：{occ_id: {country, occ_code, src_occ_id}}（无匹配则不写）。
- 幂等：处理过的 CNO 码记到 .codex_tmp/es_done.json，重跑跳过；码已存在于 ES 则跳过。
- 批次：--batch-size 每批(默认 50)，--rest 批间休息秒数(默认 0)。

运行：python -m scripts.gen_es_occupations [--batch-size 50] [--rest 0] [--limit N] [--redo]
"""
...
SOC_LIST = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "soc_es.json")
STATE = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "es_done.json")
MATCH = os.path.join(os.path.dirname(__file__), "..", ".codex_tmp", "es_ai_match.json")
...
def build_system(pool):
    ...
    listing = " | ".join(names)
    return (
        "你是西班牙劳动力市场与移民分析师，熟悉 CNO-2011 职业分类、西班牙国家就业局(SEPE)与国家统计局(INE)薪资数据、"
        "欧盟内自由流动，以及第三国技术移民路径(Tarjeta azul UE 欧盟蓝卡 / 高技能专业人才 Ley 14/2013 / "
        "Cuenta ajena 受雇工作居留 / 受规管职业的学历认证 homologación)。"
        "针对给定的西班牙 CNO 职业（标题为西文），输出务实、具体的西班牙数据。薪资用欧元(EUR)整数年薪(税前 bruto)。"
        "中文面向国际读者，不要提『华人/中文社区』。\n"
        f"职业分类只能从以下 11 类中选一个（原文照填）：{cats}\n"
        "另需从下列【现有职业候选池】中选出与该西班牙职业最接近的同一职业（用于复用分析），"
...
def build_prompt(cno, title_es, definition):
    dims = "、".join(DIMS)
    return f"""西班牙 CNO-2011 职业：
- CNO 码：{cno}
- 西文名：{title_es}

请输出该西班牙职业的 JSON，字段如下（全部必填）：
- name_en：标准英文职业名（把西文职业名译成地道、通用的英文职业名，单数，首字母大写，用于全站英文展示与匹配）
- name_zh：中文职业名
- category：从给定 11 类中选一个（原文照填）
- summary_zh / summary_en：一句话简介（中/英，各 60-140 字/词）
- forecast_note_zh：西班牙就业前景一段（60-120 字）
- trend_summary_zh：职业发展/晋升路径一段（60-120 字）
- is_migration：0/1/2（考虑欧盟自由流动；0=第三国难担保/几乎无技术移民路径；1=可走 Tarjeta azul UE/高技能专业人才(Ley 14/2013)/紧缺职业担保；2=受限/需学历认证 homologación/受规管职业）
- shortage：0 或 1（是否西班牙紧缺职业 ocupación de difícil cobertura）
- workforce_size：西班牙从业人数估计（整数）
- growth：增长/热点关键词英文数组（4 个）
- education：数组，每项{{stage:中文阶段, duration:如"4年（Grado）"或"2年（FP Grado Superior）", cost_min:整数EUR, cost_max:整数EUR, cost_note:中文}}（2-3 项，注意西班牙 FP 职教/Grado/Máster）
- qualifications：数组，每项{{qual_name:中文, issuer:机构, note:中文, is_mandatory:0或1}}（2-4 项，含执照/认证/学位/学历认证 homologación）
- salaries：数组，每项{{experience:中文如"初级（0-3年）", salary_min:整数EUR, salary_max:整数EUR, salary_note:中文}}（3 档：初/中/高，税前年薪 bruto）
- visa：数组，每项{{visa_subclass:如"Tarjeta azul UE"/"Altamente cualificado"/"Cuenta ajena"/"Busqueda empleo", visa_name:英文, description:中文}}（2-4 项西班牙工作/居留路径）
...
def validate(raw):
    ...
    out["education"] = [{"stage": (e.get("stage") or "")[:60], "duration": (e.get("duration") or "")[:50],
...
def gen(system, cno, title_es, definition):
    ...
    resp = client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": system},
                  {"role": "user", "content": build_prompt(cno, title_es, definition)}],
...
def save(cno, v):
    ...
    name_en = v["name_en"]
    OCC = {"country_code": "ES", "occ_code": cno, "anzsco_code": cno,
           "occ_code_type": "CNO", "anzsco_title": name_en, "category": v["category"],
    ...
    src = [{"source_name": "SEPE / INE", "content": "CNO 薪资与需求", "url": "https://www.sepe.es/"}]
    ...
    generate_md(cno, locale="zh-CN", country="ES")
    generate_md(cno, locale="en", country="ES")
    return occ_id
...
def run(batch_size, rest, limit, redo):
    ...
    with get_cursor() as cur:
        ...
        cur.execute("SELECT occ_code FROM occupations WHERE country_code='ES'")
        es_codes = {r["occ_code"] for r in cur.fetchall()}
    ...
    targets = [s for s in soc_list if s["soc"] not in done and s["soc"] not in es_codes]
    if limit:
        targets = targets[:limit]
    n = len(targets)
    nb = (n + batch_size - 1) // batch_size if batch_size else 1
    print(f"[es] 待生成 {n} 个(分 {nb} 批，每批 {batch_size}，批间休息 {rest}s) model={config.DEEPSEEK_MODEL}", flush=True)
    okc = fail = matched = 0
    for bi in range(nb):
        ...
        for idx, s in enumerate(chunk, 1):
            ...
            try:
                ...
                print(f"  [{tag}] → ES id={occ_id} {v['name_en']} / {v['name_zh']} [{v['category']}] mig={v['is_migration']} {mtxt}", flush=True)
