# Pharmacy Technicians — career analysis · United States

**Occupation code: 29-2052 (SOC)**

Prepare medications under pharmacist guidance, including measuring, mixing, counting, labeling, and recording prescription doses.

> **This is a non-skilled-migration occupation.** It is generally not on the skilled occupation lists and cannot be used for independent skilled migration.


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (USD) |
|---|---|---:|
| High school diploma or equivalent | 12 years | $0~$0 (Public high schools are free) |
| Pharmacy technician training program | 6 months to 2 years | $3,000~$15,000 (Community college or vocational training institution) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Certified Pharmacy Technician (CPhT) | Pharmacy Technician Certification Board (PTCB) | Required | Requires passing an exam, most states require |
| State license | State pharmacy boards | Required | Specific requirements vary by state |
| Cardiopulmonary resuscitation (CPR) certification | American Heart Association, etc. | Optional | Some employers require |

---

## 3. Job demand / competition / intensity

**Job demand: high (★★★★☆).** National workforce approx. 447,300. With an aging population and increasing chronic diseases, demand for pharmacy technicians continues to grow. BLS projects 6% employment growth from 2022-2032, faster than average.

**Competition: medium (★★½☆☆).**
**Work intensity: medium (★★½☆☆).**

---

## 4. Salary range

| Experience | Annual salary (USD) | Note |
|---|---:|---|
| Median salary | $45,750 | National full-time annual salary median (Source: US BLS OES 2025) |
| Entry level (0–3 years) | $28,000~$35,000 | Entry-level pharmacy technician |
| Mid-level (3–7 years) | $35,000~$45,000 | Experienced individuals |
| Senior (7+ years) | $45,000~$55,000 | Senior technician or supervisor |
| average salary | $46,620 | National full-time annual salary mean (Source: US BLS OES 2025) |

---

## 5. Future outlook / AI exposure

**Future prospect: medium-high (★★★½☆).**
**AI replacement risk: low (★½☆☆☆).**

AI will partially automate pharmacy technicians' medication preparation, inventory management, and documentation tasks, but interaction with patients, handling complex prescriptions, and strict compliance environments make full replacement difficult.

Can advance from entry-level pharmacy technician to senior technician or pharmacy manager, or further training to become a pharmacist.

**Tasks most exposed to AI:**

- Automatically read and enter prescription information
- Automatic counting and packaging of medications
- Automatically update inventory records and generate orders
- Automatically print and affix labels to medicine bottles

**Where AI augments the role:**

- AI-assisted checks for drug interactions and dosage errors
- AI-driven inventory demand forecasting reduces waste from expiration
- AI chatbot handles common medication inquiries
- AI automatically generates patient medication guides and repeat prescription reminders
- AI analyzes medication patterns to identify potential medication risks

**Human moat:**

- Professional judgment and exception handling for complex prescriptions
- Direct communication and consultation with patients and doctors
- Handling administrative processes like insurance and medication authorizations.
- Collaboration and emergency response within a medical team

**Skills to build:**

- Master pharmacy management systems (PMS) and AI-assisted tools
- Data analysis and medication pattern recognition
- Patient communication and medication consultation skills
- Complex prescription judgment and drug interaction knowledge
- Knowledge of regulatory compliance and insurance processes

**Key growth areas:**

- Aging population
- Chronic disease management
- Retail pharmacy expansion
- Specialty pharmacy growth

---

## 6. Migration pathways / PR

**This is a non-skilled-migration occupation.** It is generally not on the skilled occupation lists and cannot be used for independent skilled migration.

| Visa | Description |
|---|---|
| Subclass H-1B (Specialty Occupation) | Usually does not require a bachelor's degree; H-1B is uncommon. |
| Subclass Green Card (PERM) (Employment-Based Immigration) | Employer-sponsored permanent residency is difficult as the occupation does not require a higher education degree. |

**PR friendliness: low (★☆☆☆☆).**

**PR difficulty: very high (★★★★½).**

---

## 7. Who is / isn't suited

**Who is suited**
- Detail-oriented, strong sense of responsibility
- Liking to work in a medical environment
- Good communication skills

**Who is not suited**
- Those seeking high-paying careers
- People unwilling to stand for long periods

---

## 8. FAQ

**Q: What are the salary levels for pharmacy technicians?**
A: Median annual salary approximately $36,000 USD, entry-level around $30,000, senior can reach $50,000 or more.

**Q: Is it easy for pharmacy technicians to immigrate to the US via skilled migration?**
A: Very difficult. This occupation typically does not require a bachelor's degree, does not meet common H-1B requirements, and employer-sponsored green cards are also difficult.

**Q: What certifications are required?**
A: Most states require passing the PTCB exam to obtain CPhT certification; some states also require a state license.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | low (★☆☆☆☆) |
| Learning difficulty | low (★☆☆☆☆) |
| Certification difficulty | medium (★★½☆☆) |
| Job demand | high (★★★★☆) |
| Competition | medium (★★½☆☆) |
| Income level | low (★½☆☆☆) |
| Work intensity | medium (★★½☆☆) |
| Future prospect | medium-high (★★★½☆) |
| AI replacement risk | low (★½☆☆☆) |
| PR friendliness | low (★☆☆☆☆) |
| PR difficulty | very high (★★★★½) |

---

## Data sources

- U.S. BLS OEWS, O*NET, SOC
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
