# Cashier — career analysis · South Korea

**Occupation code: 61611 (KECO)**

A cashier is a professional who calculates the price of products purchased by customers at a store or sales outlet, receives payment, or issues receipts.

> **This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (KRW) |
|---|---|---:|
| High school diploma | 0 years | $0~$0 (No special educational requirements) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Cashier certificate | Korea Research Institute for Vocational Education and Training | Optional | Optional, additional points for employment |
| POS system proficiency | Private certification | Optional | Mainly acquired on-site |

---

## 3. Job demand / competition / intensity

**Job demand: low (★½☆☆☆).** National workforce approx. 450,000. Employment is declining due to the spread of self-checkout machines and kiosks, but demand is expected to remain at some stores requiring face-to-face service.

**Competition: medium (★★½☆☆).**
**Work intensity: low (★½☆☆☆).**

---

## 4. Salary range

| Experience | Annual salary (KRW) | Note |
|---|---:|---|
| Beginner (0-3 years) | $20,000,000~$25,000,000 | Based on hourly wage, for 40 hours per week |
| Intermediate (3-7 years) | $25,000,000~$30,000,000 | Increases with experience |
| Advanced (7+ years) | $28,000,000~$35,000,000 | Store manager level |

---

## 5. Future outlook / AI exposure

**Future prospect: low (★☆☆☆☆).**
**AI replacement risk: high (★★★★☆).**

With the introduction of self-checkout and kiosks, a significant portion of cashier duties have been automated, greatly reducing employment. Simple repetitive tasks have been replaced by AI and automation, but customer service and handling complex situations still require human intervention.

Starting as a new cashier, responsibilities gradually expand to customer service or inventory management, and with experience, one can advance to store manager or salesperson.

**Tasks most exposed to AI:**

- Scanning and calculating product prices
- Processing cash and card payments
- Issuing receipts

**Where AI augments the role:**

- Handling customer inquiries and complaints
- Guiding customers to products in the store
- Monitoring kiosks and troubleshooting

**Human moat:**

- Face-to-face communication skills
- Ability to handle complex situations
- Trustworthiness and friendliness

**Skills to build:**

- Customer service skills
- POS system operation
- Basic accounting and inventory management

**Key growth areas:**

- Automation
- Kiosk introduction
- Self-checkout
- Alternative services

---

## 6. Migration pathways / PR

**This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).

| Visa | Description |
|---|---|
| Subclass E-9 (E-9 (Non-professional Employment)) | Some retail sectors have demand for unskilled labor, but cashier is not the main visa. |
| Subclass F-4 (F-4 (Overseas Korean)) | Overseas Koreans can work as cashiers without employment restrictions. |

**PR friendliness: high (★★★★☆).**

**PR difficulty: low (★☆☆☆☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- Detail-oriented and quick with numbers
- Friendly in customer service
- Patient with simple repetitive tasks

**Who is not suited**
- Someone who wants stimulation and change
- Someone who finds using digital devices difficult

---

## 8. FAQ

**Q: What is the average salary of a cashier?**
A: Around 20-25 million KRW for entry level, varying with experience and work hours.

**Q: Can foreigners work as cashiers?**
A: Possible with E-9 (Non-professional Employment) visa, but not a mainstream route. F-4 (Overseas Koreans) can work freely.

**Q: Is there room for promotion as a cashier?**
A: Yes, you can be promoted to store manager or sales associate, with career growth through additional training.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | very low (½☆☆☆☆) |
| Learning difficulty | very low (½☆☆☆☆) |
| Certification difficulty | very low (½☆☆☆☆) |
| Job demand | low (★½☆☆☆) |
| Competition | medium (★★½☆☆) |
| Income level | low (★☆☆☆☆) |
| Work intensity | low (★½☆☆☆) |
| Future prospect | low (★☆☆☆☆) |
| AI replacement risk | high (★★★★☆) |
| PR friendliness | high (★★★★☆) |
| PR difficulty | low (★☆☆☆☆) |

---

## Data sources

- KOSTAT, KECO, KSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
