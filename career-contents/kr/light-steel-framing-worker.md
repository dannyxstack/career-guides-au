# Light Steel Framing Worker — career analysis · South Korea

**Occupation code: 70121 (KECO)**

A specialized construction worker responsible for installing, assembling, and dismantling light steel frames, primarily for building frameworks and roof structures.


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (KRW) |
|---|---|---:|
| High school diploma or above | 3 years | $0~$500,000 (Training costs may apply) |
| Craftsman certificate | 1-2 years | $1,000,000~$3,000,000 (Including tuition fees) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Construction/Welding Certification | Human Resources Development Service of Korea | Required | Required for welding tasks |
| Safety and health training completion | Ministry of Employment and Labor | Required | Mandatory for construction sites |
| Steel Structure & Reinforcement Craftsman | Human Resources Development Service of Korea | Optional | Related certifications for light steel framing, bonus points |

---

## 3. Job demand / competition / intensity

**Job demand: high (★★★★☆).** National workforce approx. 35,000. Closely tied to the construction industry; demand remains steady due to the trend toward lightweight buildings, but labor shortages are expected to persist due to aging workforce and lack of new entrants.

**Competition: medium (★★½☆☆).**
**Work intensity: very high (★★★★½).**

---

## 4. Salary range

| Experience | Annual salary (KRW) | Note |
|---|---:|---|
| Entry-level (0-3 years) | $24,000,000~$30,000,000 | Excluding night shift/hazard pay |
| Intermediate (4-7 years) | $32,000,000~$40,000,000 | Additional pay for certifications |
| Advanced (8+ years) | $40,000,000~$55,000,000 | Site manager level |

---

## 5. Future outlook / AI exposure

**Future prospect: medium-high (★★★½☆).**
**AI replacement risk: low (★☆☆☆☆).**

Due to AI and automation, simple repetitive tasks (material handling, cutting, etc.) are reduced, but on-site assembly, welding, and safety management require human judgment and skill, making full replacement difficult.

Career progression: entry-level → intermediate → advanced → site manager or master craftsman; relevant certifications and experience are crucial.

**Tasks most exposed to AI:**

- Material handling and cutting
- Simple assembly and bolt fastening
- Basic surveying and marking

**Where AI augments the role:**

- Design optimization through pre-simulation
- Monitoring of robot welding and automatic cutting equipment
- Site management through drones and 3D scanning

**Human moat:**

- Site adaptability and immediate problem-solving skills
- Precision welding and assembly skills for complex structures
- Safety management and team communication

**Skills to build:**

- Robot welding operation
- Use of BIM tools
- Operation of smart construction equipment
- Basic programming (automation equipment)

**Key growth areas:**

- Increase in demand for light steel frames
- Lightweight building construction
- Utilization of E-9 visa
- Shortage of skilled workers

---

## 6. Migration pathways / PR


| Visa | Description |
|---|---|
| Subclass E-9 (E-9 (Non-professional Employment)) | Target: Low-skilled labor, Construction Employment Permit System applies |
| Subclass E-7 (E-7 (Specific Activities)) | Skilled labor, may require certifications |
| Subclass F-2 (F-2 (Residence)) | Long-term residents, no work restrictions |

**PR friendliness: medium-high (★★★☆☆).**

**PR difficulty: medium-high (★★★½☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- People who are physically fit and prefer outdoor work
- People with dexterous hands who enjoy working with tools
- People who strictly follow safety rules

**Who is not suited**
- People with a fear of heights
- People who prefer precise paperwork

---

## 8. FAQ

**Q: What is the starting salary for a light gauge steel framer?**
A: Entry-level pre-tax annual salary is around 24-30 million KRW, increasing with experience and certifications.

**Q: What visa does a foreigner need to work as a light gauge steel framer?**
A: Requires E-9 (Non-professional Employment) visa (Employment Permit System) or E-7 (Specific Activities) visa, and skilled workers can switch to F-2 (Residence).

**Q: Can a light steel frame worker switch to other jobs?**
A: By utilizing welding skills, one can switch to general steel frame worker, structure installer, etc., and also advance to site management positions.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | medium-low (★★☆☆☆) |
| Learning difficulty | medium (★★½☆☆) |
| Certification difficulty | low (★½☆☆☆) |
| Job demand | high (★★★★☆) |
| Competition | medium (★★½☆☆) |
| Income level | medium (★★½☆☆) |
| Work intensity | very high (★★★★½) |
| Future prospect | medium-high (★★★½☆) |
| AI replacement risk | low (★☆☆☆☆) |
| PR friendliness | medium-high (★★★☆☆) |
| PR difficulty | medium-high (★★★½☆) |

---

## Data sources

- KOSTAT, KECO, KSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
