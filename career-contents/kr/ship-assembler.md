# Ship Assembler — career analysis · South Korea

**Occupation code: 81731 (KECO)**

A skilled technical professional who welds, cuts, and assembles blocks that make up a ship's hull to construct large vessels.

> **This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (KRW) |
|---|---|---:|
| High School Diploma | 0 years | $0~$0 (Free tuition due to compulsory education) |
| Associate Degree in Shipbuilding | 2 years | $6,000,000~$12,000,000 (Tuition KRW 3-6 million per year) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Welding Technician | Human Resources Development Service of Korea | Optional | Essential for ship welding; preferential treatment for certificate holders |
| Assembly Technician | Human Resources Development Service of Korea | Optional | Helps with understanding hull assembly process |
| Special Welding Technician | Human Resources Development Service of Korea | Optional | Advantageous for high-value ship welding |

---

## 3. Job demand / competition / intensity

**Job demand: medium (★★½☆☆).** National workforce approx. 45,000. Employment is unstable due to restructuring in the shipbuilding industry and fluctuations in orders, but demand for high-value vessels like LNG carriers persists, requiring continuous need for skilled workers.

**Competition: medium-high (★★★½☆).**
**Work intensity: high (★★★★☆).**

---

## 4. Salary range

| Experience | Annual salary (KRW) | Note |
|---|---:|---|
| Entry Level (0-3 years) | $30,000,000~$40,000,000 | Based on shipyard regular employee salary; base pay + allowances |
| Mid-level (4-9 years) | $42,000,000~$55,000,000 | Additional allowances for special welding certifications |
| Advanced (10+ years) | $55,000,000~$75,000,000 | Includes performance bonuses when promoted to foreman |

---

## 5. Future outlook / AI exposure

**Future prospect: medium-low (★★☆☆☆).**
**AI replacement risk: medium (★★★☆☆).**

AI and automation assist and enhance welding and cutting processes in ship block assembly, but complex curved welding and quality judgment still rely on humans, so jobs will not be fully replaced.

Entry-level welders can advance to intermediate assemblers, foremen, and process managers; those with over 10 years of experience at large shipyards can become technical supervisors.

**Tasks most exposed to AI:**

- Automated welding
- Simple cutting
- Repetitive transport work
- Basic assembly

**Where AI augments the role:**

- Welding quality inspection
- Precision curved welding
- Robot operation monitoring
- Assembly process optimization

**Human moat:**

- High-difficulty manual welding
- On-site adaptive problem-solving
- Visual inspection and repair judgment
- Team collaboration and safety management

**Skills to build:**

- Welding robot programming
- 3D CAD reading
- Data-driven quality management
- Understanding Smart Shipyard Systems

**Key growth areas:**

- LNG carrier
- Eco-friendly ship
- Automated welding
- Smart shipyard

---

## 6. Migration pathways / PR

**This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).

| Visa | Description |
|---|---|
| Subclass E-9 (Non-professional Employment (E-9)) | Used for hiring unskilled shipbuilding workers; skilled workers can switch to E-7. |
| Subclass E-7 (Specific Activities (E-7)) | For skilled ship assemblers, employer recommendation required. |
| Subclass F-2 (Residence (F-2)) | After a certain period on E-7, can apply for permanent residency. |

**PR friendliness: low (★½☆☆☆).**

**PR difficulty: medium-high (★★★½☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- Physically fit and able to tolerate outdoor, high-altitude work
- Detail-oriented with interest in welding and assembly
- Team player who follows safety rules

**Who is not suited**
- Prefers office work over precision tasks
- Cannot tolerate noise and dust environment

---

## 8. FAQ

**Q: What is the starting salary for a ship assembler?**
A: At large shipyards, about 30-40 million KRW, varies with overtime and holiday pay.

**Q: Can foreigners work as ship assemblers?**
A: Yes, it's possible. You can enter on an E-9 visa, switch to an E-7 visa after gaining skill, and apply for an F-2 (Residence) visa after certain experience.

**Q: What is the long-term career path for a ship assembler?**
A: You can advance from skilled worker to foreman or process manager, or obtain advanced welding certifications to become a specialized ship welding expert.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | medium-high (★★★½☆) |
| Learning difficulty | medium-high (★★★★☆) |
| Certification difficulty | medium (★★½☆☆) |
| Job demand | medium (★★½☆☆) |
| Competition | medium-high (★★★½☆) |
| Income level | medium (★★★☆☆) |
| Work intensity | high (★★★★☆) |
| Future prospect | medium-low (★★☆☆☆) |
| AI replacement risk | medium (★★★☆☆) |
| PR friendliness | low (★½☆☆☆) |
| PR difficulty | medium-high (★★★½☆) |

---

## Data sources

- KOSTAT, KECO, KSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
