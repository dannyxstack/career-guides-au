# 嵌入式/固件工程师（Embedded / Firmware Engineer）职业分析 · ES

**职业代码：2442 – Embedded / Firmware Engineer。**
...
**职位需求量：高（★★★★☆）。** 全国从业人数约 12,000。在西班牙，设备联网化与汽车/医疗/国防电子推动嵌入式需求，嵌入式/固件工程师供需偏紧，前景稳健。

| 平台 | 实时挂牌量（约） | 备注 |
|---|---:|---|
| InfoJobs | 150~500 个 | 嵌入式/固件工程师及相邻岗位（西班牙） |
| Indeed ES | 100~350 个 | 嵌入式/固件工程师及相邻岗位（西班牙） |
| LinkedIn | 200~600 个 | 嵌入式/固件工程师及相邻岗位（西班牙） |
...
**发展前景：很好（★★★★½）。** 西班牙边缘 AI、功能安全与设备安全成为热点，懂底层调试与实时约束的固件工程师稀缺；AI 生成样板，硬件底层仍依赖人。
**AI替代风险：中低（★★☆☆☆）。**
