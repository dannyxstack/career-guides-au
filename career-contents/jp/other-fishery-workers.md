# Other Fishery Workers — career analysis · Japan

**Occupation code: 489 (JSCO)**

Engaged in fishing but performing tasks not classified under specific subcategories such as fishing, aquaculture, or net making.

> **This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (JPY) |
|---|---|---:|
| Fisheries High School | 3 years | $0~$500,000 (Public high schools are almost free) |
| Vocational School (Fisheries/Marine Sciences) | 2 years | $1,000,000~$2,000,000 (Tuition fees, etc.) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Small Vessel Operator License | Ministry of Land, Infrastructure, Transport and Tourism | Required | Required for operating fishing boats |
| Fisheries Radio Operator Qualification | Ministry of Internal Affairs and Communications | Required | Needed for radio communication duties |
| Marine Engineer License (Fishing) | Ministry of Land, Infrastructure, Transport and Tourism | Optional | Mandatory for large fishing vessels |

---

## 3. Job demand / competition / intensity

**Job demand: low (★½☆☆☆).** National workforce approx. 12,000. The number of fishery workers is declining, but there is steady demand due to aging. Shortage of workers in coastal fisheries is a challenge.

**Competition: low (★☆☆☆☆).**
**Work intensity: high (★★★★☆).**

---

## 4. Salary range

| Experience | Annual salary (JPY) | Note |
|---|---:|---|
| Entry-level (0-3 years) | $2,500,000~$3,500,000 | Annual income example, commission based on catch |
| Mid-level (4-9 years) | $3,500,000~$5,000,000 | Varies by experience and position |
| Senior-level (10+ years) | $4,500,000~$7,000,000 | Captain/manager level |

---

## 5. Future outlook / AI exposure

**Future prospect: low (★☆☆☆☆).**
**AI replacement risk: low (★☆☆☆☆).**

AI/automation (fish finders, auto-steering, etc.) improve some tasks, but many tasks like gear handling and fish processing remain labor-dependent, making full substitution difficult.

The path to becoming a skilled fisherman is long, taking over 10 years to independence. Experience at fisheries cooperatives or companies, plus boat licenses and fishing rights, are needed.

**Tasks most exposed to AI:**

- Fish finding
- Voyage planning optimization

**Where AI augments the role:**

- Catch data analysis
- Determining net placement locations

**Human moat:**

- Manual adjustment and repair of fishing gear
- Experiential judgment of weather and sea conditions
- Fish species identification and quality assessment

**Skills to build:**

- Marine data analysis
- Fisheries ICT equipment operation
- Aquaculture technology

**Key growth areas:**

- Fisheries resource management
- Aquaculture technology
- Fisheries successors
- Smart fisheries

---

## 6. Migration pathways / PR

**This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).

| Visa | Description |
|---|---|
| Subclass 特定技能1号 (Specified Skilled Worker (i) (Fishery)) | Eligible to work in fishery sector; max stay 5 years |
| Subclass 技能実習 (Technical Intern Training (Fishery)) | Skills transfer; max 5 years |
| Subclass 技術・人文知識・国際業務 (Engineer/Specialist in Humanities/International Services) | Fishery engineers, etc., advanced knowledge required |

**PR friendliness: low (★☆☆☆☆).**

**PR difficulty: high (★★★★☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- Those confident in physical strength
- Those who love nature and are patient
- Those who don't mind early morning/night shifts

**Who is not suited**
- Those who desire a regular lifestyle
- Those who prefer land-based work

---

## 8. FAQ

**Q: What is the average annual income of fishery workers?**
A: 2.5-3.5M JPY in first year, over 5M possible with experience. Varies greatly with catch.

**Q: How can foreigners work in fishery?**
A: The main routes are Specified Skilled Worker (i) (Fisheries) or Technical Intern Training. JLPT N4 level required.

**Q: How to become independent in fisheries?**
A: A boat license, fishing rights acquisition, and fishing boat purchase funds are needed. Joining a fisheries cooperative is often required.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | medium-high (★★★½☆) |
| Learning difficulty | medium (★★½☆☆) |
| Certification difficulty | medium (★★½☆☆) |
| Job demand | low (★½☆☆☆) |
| Competition | low (★☆☆☆☆) |
| Income level | low (★½☆☆☆) |
| Work intensity | high (★★★★☆) |
| Future prospect | low (★☆☆☆☆) |
| AI replacement risk | low (★☆☆☆☆) |
| PR friendliness | low (★☆☆☆☆) |
| PR difficulty | high (★★★★☆) |

---

## Data sources

- MHLW / e-Stat, JSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
