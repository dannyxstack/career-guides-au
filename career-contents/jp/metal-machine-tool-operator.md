# Metal Machine Tool Operator — career analysis · Japan

**Occupation code: 523 (JSCO)**

A specialist who operates machine tools such as lathes, milling machines, and grinders to cut and process metal parts. Skilled craftsmanship supporting the manufacturing industry base.


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (JPY) |
|---|---|---:|
| High school graduation | 3 years | $0~$3,000,000 (Public high school: tuition-free eligibility) |
| Vocational school (Mechanical Engineering) | 2 years | $1,000,000~$3,000,000 (Private: including tuition and material costs) |
| Vocational training school (standard vocational training) | 1-2 years | $0~$500,000 (Hello Training; tuition may be free) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| Skills Certification (Metal Machine Tool) Grade 1 | Ministry of Health, Labour and Welfare | Optional | Proof of advanced skills; advantageous for promotion |
| Skills Certification (Metal Machine Tool) Grade 2 | Ministry of Health, Labour and Welfare | Optional | Proof of basic skills; recommended by many companies |
| NC Machine Tool Operator Skills Certification | Japan Machine Tool Builders' Association | Optional | Operation capability for NC lathes and machining centers |
| Gas Welding Skills Training | Prefectural Labour Bureau | Optional | May be required |

---

## 3. Job demand / competition / intensity

**Job demand: high (★★★★☆).** National workforce approx. 320,000. Demand persists due to domestic manufacturing resurgence and automotive/semiconductor needs, but employment is declining due to youth shortage and automation.

**Competition: medium (★★½☆☆).**
**Work intensity: medium-high (★★★½☆).**

---

## 4. Salary range

| Experience | Annual salary (JPY) | Note |
|---|---:|---|
| Entry (0–3 years) | $3,000,000~$4,000,000 | Annual salary ¥3–4 million, if skills are basic |
| Intermediate (4–10 years) | $4,000,000~$5,500,000 | Annual salary ¥4–5.5 million, approx. skill test level 2 |
| Expert (11+ years) | $5,000,000~$7,000,000 | Annual salary ¥5–7 million, plus extra for skill test level 1 & multi-skilling |

---

## 5. Future outlook / AI exposure

**Future prospect: medium-high (★★★☆☆).**
**AI replacement risk: medium (★★½☆☆).**

Impact of AI and automation is ongoing. Automation of work instructions via CAD/CAM and robot loading/unloading are advancing, but complete substitution is difficult for complex one-off items and changeover decisions that require human judgment.

Career advancement possible through skills certification and multi-skilling. Path from NC/MC operator to programming and production management.

**Tasks most exposed to AI:**

- Simple manual lathe work
- Repetitive mass production processes
- Automation-compatible tool change processes

**Where AI augments the role:**

- Creating machining programs via CAM
- On-machine measurement data analysis
- AI-based tool wear prediction

**Human moat:**

- Processing complex one-off shapes
- Changeover decision-making
- Troubleshooting skills

**Skills to build:**

- NC Programming
- CAD/CAM Operations
- Robot Teaching
- Multi-process Mastery

**Key growth areas:**

- Automotive parts processing
- Aircraft parts processing
- Semiconductor manufacturing equipment parts
- Die and mold manufacturing

---

## 6. Migration pathways / PR


| Visa | Description |
|---|---|
| Subclass 特定技能1号 (Specified Skilled Worker (i) (Casting, industrial machinery, electrical/electronic/information manufacturing)) | Foreigners can work in metal processing. Requires skills test & Japanese test. Max. residency 5 years. |
| Subclass 特定技能2号 (Specified Skilled Worker (ii) (Casting, industrial machinery, electrical/electronic/information manufacturing)) | For skilled experts. No residency renewal limit; family accompaniment & PR application possible. |
| Subclass 技術・人文知識・国際業務 (Engineer/Specialist in Humanities/International Services (Engineering)) | Covers advanced knowledge work like NC programming & production technology. Requires university degree or work experience. |

**PR friendliness: medium-high (★★★½☆).**

**PR difficulty: medium (★★½☆☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- People interested in manufacturing with dexterous hands
- People who like meticulous and precise work
- People who are physically confident and can tolerate repetitive tasks

**Who is not suited**
- People who prefer desk work
- People who find routine work boring

---

## 8. FAQ

**Q: What is the annual income of metal machine tool operators?**
A: For beginners, annual income is about 3-4 million yen, for intermediate 4-5.5 million yen, and for skilled workers 5-7 million yen. It can increase further with skill certification and multi-skilling.

**Q: What visa do foreigners need to work in this occupation?**
A: Specified Skilled Worker (i)/(ii) (molding/industrial machinery, etc.) or Engineer/Specialist in Humanities/International Services (Engineering) applies. Japanese language ability and skill tests are required.

**Q: What are the career advancement paths?**
A: After obtaining grade 1 or 2 of the skill test, there are paths to become an NC programmer or production manager. Acquiring multi-skills like welding and assembly is advantageous.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | medium-high (★★★★☆) |
| Learning difficulty | medium-high (★★★½☆) |
| Certification difficulty | medium-high (★★★½☆) |
| Job demand | high (★★★★☆) |
| Competition | medium (★★½☆☆) |
| Income level | medium (★★½☆☆) |
| Work intensity | medium-high (★★★½☆) |
| Future prospect | medium-high (★★★☆☆) |
| AI replacement risk | medium (★★½☆☆) |
| PR friendliness | medium-high (★★★½☆) |
| PR difficulty | medium (★★½☆☆) |

---

## Data sources

- MHLW / e-Stat, JSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
