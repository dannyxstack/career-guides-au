# Orthoptist, Speech-Language Pathologist — career analysis · Japan

**Occupation code: 145 (JSCO)**

Orthoptists and speech-language pathologists are medical technologists specializing in rehabilitation for visual impairments and speech/hearing disorders, respectively.

> **This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).


---

## 1. Education path / duration / cost

| Stage | Duration | Cost (JPY) |
|---|---|---:|
| Vocational school (Orthoptist) | 3 years | $3,000,000~$4,000,000 (Total cost for private vocational schools) |
| University (Speech-Language Pathologist) | 4 years | $4,000,000~$6,000,000 (Less than half at national/public universities) |

---

## 2. Qualifications & licensing

| Qualification | Issuer | Required | Note |
|---|---|---|---|
| National Orthoptist License | Ministry of Health, Labour and Welfare | Required | Licensed after passing national exam |
| National Speech-Language Pathologist License | Ministry of Health, Labour and Welfare | Required | Licensed after passing national exam |
| Certified Orthoptist | Japanese Association of Orthoptists | Optional | Voluntary certification for advanced expertise |

---

## 3. Job demand / competition / intensity

**Job demand: high (★★★★☆).** National workforce approx. 65,000. Demand is steadily increasing with aging. Speech-language pathologists' roles especially expand in community-based integrated care systems. Orthoptists see stable needs from children to the elderly.

**Competition: low (★½☆☆☆).**
**Work intensity: medium (★★½☆☆).**

---

## 4. Salary range

| Experience | Annual salary (JPY) | Note |
|---|---:|---|
| Entry (0–3 years) | $3,200,000~$3,800,000 | Hospital employment, incl. bonuses annual income |
| Mid-career (4–10 years) | $4,000,000~$5,000,000 | Varies by experience and facility size |
| Senior (10+ years) | $4,800,000~$6,000,000 | Increases with management roles or certification |

---

## 5. Future outlook / AI exposure

**Future prospect: high (★★★★☆).**
**AI replacement risk: low (★☆☆☆☆).**

AI is increasingly used for rehabilitation planning and training assistance, but patient communication and individual care still rely on humans. AI is expected to improve efficiency and treatment quality.

Orthoptists gain expertise through clinical experience and certifications. Speech-language pathologists expand careers from hospitals to home-visit rehab. Both can pursue research via graduate school.

**Tasks most exposed to AI:**

- Automated visual field test analysis
- Partial automation of standard language training

**Where AI augments the role:**

- AI-assisted visual and hearing tests
- Automatic recording and analysis of rehabilitation progress

**Human moat:**

- Building trust with patients
- Clinical judgment for individual adjustments
- Non-verbal communication

**Skills to build:**

- Operation of AI rehabilitation tools
- Data analysis and interpretation skills
- Multidisciplinary collaboration skills

**Key growth areas:**

- Community-based rehabilitation
- Home healthcare
- Pediatric developmental support
- Dementia prevention

---

## 6. Migration pathways / PR

**This is a restricted skilled-migration occupation (employer-sponsored / regional agreements only).** It is not on the independent skilled lists (e.g. 189/190).

| Visa | Description |
|---|---|
| Subclass 技人国 (Engineer/Specialist in Humanities/International Services) | Even foreigners can work in medical institutions after obtaining both national certifications. However, individual审查 determines if the job scope applies. |
| Subclass 特定技能1号 (Specified Skilled Worker (i) (Caregiving Field)) | Not covered under Specified Skilled Worker in caregiving. Orthoptists and speech-language therapists are excluded. |
| Subclass 高度専門職 (Highly Skilled Professional (i) (a)) | Graduates of graduate schools in research/education etc. may qualify. Rare in practice. |

**PR friendliness: medium-high (★★★☆☆).**

**PR difficulty: medium-high (★★★½☆).**

---

## 7. Who is / isn't suited

**Who is suited**
- People who enjoy interacting with others
- Patient people who can steadily build up rehabilitation
- People who want to deepen medical expertise

**Who is not suited**
- People who seek short-term results
- People who are not good at flexible communication

---

## 8. FAQ

**Q: What is the annual income of orthoptists and speech-language therapists?**
A: Starting salary is about 3.2 to 3.8 million yen annually; with 10+ years of experience, about 4.8 to 6 million yen. Varies by hospital size and region.

**Q: How can foreigners work as orthoptists or speech-language therapists in Japan?**
A: Requires Japanese national certification. After studying at a training school and passing the national exam, work is possible with an Engineer/Specialist in Humanities/International Services visa. However, Japanese language proficiency is mandatory.

**Q: Is it possible to obtain both orthoptist and speech-language-hearing therapist certifications?**
A: Possible, but each requires a 3–4 year training program, costing time and money. Few people hold both.


---

## Quick summary

| Dimension | Rating |
|---|---|
| Learning duration | medium-high (★★★☆☆) |
| Learning difficulty | medium (★★½☆☆) |
| Certification difficulty | medium-high (★★★☆☆) |
| Job demand | high (★★★★☆) |
| Competition | low (★½☆☆☆) |
| Income level | medium-low (★★☆☆☆) |
| Work intensity | medium (★★½☆☆) |
| Future prospect | high (★★★★☆) |
| AI replacement risk | low (★☆☆☆☆) |
| PR friendliness | medium-high (★★★☆☆) |
| PR difficulty | medium-high (★★★½☆) |

---

## Data sources

- MHLW / e-Stat, JSCO (LLM→ISCO map)
- AI exposure: ILO Working Paper 140 (Generative AI exposure index, CC BY 4.0) + Eloundou et al. "GPTs are GPTs" (OpenAI, MIT)
